...
def caption_llava_onevision(
    ...
    """Iterate a Caption CSV and write answers generated by LLaVA‑OneVision."""
    ...
    with open_csv_writer(csv_out, fields) as (writer, fh):
        bar = tqdm(total=len(df), unit="caption", desc="LLaVA‑OneVision Caption")
        try:
...
def main():
    ...
    out_csv = out_dir / (
        f"{args.model.split('/')[-1]}_caption_{args.mode}_t-{args.temperature}_p-{args.top_p}.csv"
    )

    caption_llava_onevision(
        Path(input_csv),
        out_csv,
        model,
        processor,
